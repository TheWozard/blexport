...
- **`cmd/export.go`** (backed by `internal/blender`, `internal/manifest`) — the CLI entry point, a plain Go binary with no `bpy` dependency. Finds `.blend` files, checks manifest staleness (`.blend` mtime vs `.json` mtime), and for each stale asset shells out to `blender <asset>.blend --background --python <tmpdir>/blender_scene_export.py -- <out_dir> <px_per_unit> <png_px_per_unit> <pitch_deg>`, where `<tmpdir>` is a fresh directory holding a copy of `blender_scene_export.py`, written out from a `go:embed`'d copy (Blender's `--python` flag needs a real path on disk, and `go:embed` can't reach outside its own package directory — so `internal/blender/blender_scene_export.py` is the only copy of this file; don't recreate a copy at the repo root). Reads back the JSON that script wrote and reports per-asset stats.
...
- All mesh objects (anything not an Empty/Camera/Light) are combined into one world-space bounding box — that's what auto-framed cameras fit to. Objects flagged `Object.is_holdout` are excluded from this (see below).
- Four standard 45°-isometric directions are always rendered: NE/NW/SE/SW (compass azimuth in Blender's XY plane, +Y = north). If the scene has a Camera object named after one of those (case-insensitive), its own transform/lens/resolution is used as-is; otherwise one is auto-created and orthographically fit to the asset's actual mesh silhouette (evaluated vertices, modifiers applied), not the bounding box — the box's own corners sit in empty space for non-boxy shapes, and an isometric camera looks straight at that space, which reads as excess padding.
...
- Every material in the scene must be a single Diffuse BSDF wired directly into its Material Output's Surface input — a hard error (`die()`) otherwise, no fallback. `render_views()` derives two throwaway `Material.copy()`s from each source material per view (`build_bake_materials()`), swapped into every mesh object's slots for that one render (`swap_materials()`) and always removed afterward: an **albedo** pass for `image` (whatever fed the Diffuse BSDF's Color input, plugged into an Emission shader instead — flat, self-illuminated, no scene lighting/shadow baked in; the engine is expected to light it at runtime) and a **normal** pass for `normal_image` (whatever fed the Diffuse BSDF's Normal input — a Bump/Normal Map chain, or nothing, meaning the implicit geometric normal — run through one Vector Transform World→Camera → multiply by `(1,1,-1)` to undo a Z-negation quirk in Blender's Vector Transform node → encode to `[0,1]` → Emission). Only one Vector Transform is needed, inserted at that single Normal-input socket, since everything upstream of it already resolves to a finished world-space normal by Blender's own BSDF convention. The render forces `view_settings.view_transform = "Raw"` for the normal pass only, so AgX/Filmic doesn't distort the encoded values. Both bakes are always present now — there's no scene without them.
