...
- `views.<key>` — one entry per rendered direction/camera, keyed by direction label (`ne`/`nw`/`se`/`sw`) or camera object name (e.g. `icon` above). Extra-camera views only appear when the scene has extra cameras.
