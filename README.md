...
- **Materials** — every material in the scene must be a single Diffuse BSDF wired directly into its Material Output's Surface input; anything else is a hard export error, with no fallback. That one node is what both `image` and `normal_image` are baked from, in place of the scene's own lighting.
- **Albedo (`image`)** — each material's Color input is baked through an Emission shader instead of the Diffuse BSDF, so `image` is that surface's own flat, self-illuminated base color — no scene lighting, shadow, or shading gradient baked in. A consuming engine is expected to apply all real lighting itself at runtime, using this plus `normal_image`.
- **Normal maps (`normal_image`)** — each material's Normal input (a Bump/Normal Map node chain, or nothing, meaning the implicit geometric normal) is baked through the same kind of Emission swap, writing a `<view>_normal.png` next to that view's regular image and adding a `normal_image` key to its manifest entry — always present now. The bake outputs each surface's normal in *that view's own screen space* — `+X` right, `+Y` up, `+Z` toward the viewer — via a single `Vector Transform` (World → Camera) inserted at that Normal-input socket (everything upstream of it, however elaborate, already resolves to a finished world-space normal by Blender's own BSDF convention, so no other node in the graph needs its own transform) → **multiply by `(1, 1, -1)`** (Blender's Vector Transform node negates Z relative to the camera's own local axes — an empirically-verified quirk, not documented anywhere — so this flips it back) → multiply-add by `0.5`/`0.5` to map `[-1, 1]` into `[0, 1]` → `Emission`. The render forces the scene's view transform to `Raw` for this one pass, since the encoded values are data, not a color to be graded by AgX/Filmic/etc — the albedo pass is unaffected. For an asset with a front/back split (see below), the normal pass fully hides the holdout object(s) — same as the `image` render itself — rather than using their Holdout flag, so `normal_image` covers the rest of the asset's full, unoccluded surface, matching what `image` shows.
